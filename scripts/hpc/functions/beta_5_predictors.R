...
beta_values <- seq(from = 0.10, to = 1.6, by = 0.001)

# Calculate and store "a" values for different scenarios
beta_value_1end_normal <-calculate_auc5("1normal")
beta_value_1end_interaction <- calculate_auc5("1interaction")
beta_value_2end_normal <- calculate_auc5("2normal")
beta_value_2end_normalinteraction <- calculate_auc5("2normalinteraction")
beta_value_2end_interaction <- calculate_auc5("2interaction")

# Create a data frame to store all the results
beta_values_5predictors_covariance <- data.frame(
  Scenario = c(
...
write.csv(beta_values_5predictors_covariance, file = "data/datasets/beta_values/5predictors_covariance.csv", row.names = FALSE)